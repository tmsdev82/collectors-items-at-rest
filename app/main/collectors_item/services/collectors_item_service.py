...
def get_collectors_items():
    ...
    try:
        ...
        logger.exception(e)
        response_object = {"status": "failed", "message": "an error occurred"}
        return response_object, 500


def create_collectors_item(collectors_item_data):
    logger.debug(
        ">>>> collectors_item_data: {}".format(json.dumps(collectors_item_data, indent=2))
    )
    try:
...
def update_collectors_item_by_id(collectors_item_id, collectors_item_data):
    logger.debug(
        ">>>> collectors_item_id {}, collectors_item_data: {}".format(
            collectors_item_id, json.dumps(collectors_item_data, indent=2)
        )
    )
    try:
        collectors_item_to_update = (
            _db.session.query(CollectorsItemModel)
            .filter(CollectorsItemModel.id == collectors_item_id)
            .first()
        )
        ...
        if not collectors_item_to_update:
            ...
            response_object = {"status": "failed", "message": error_msg}
        ...
        if collectors_item_data.get("collectors_item_type"):
            collectors_item_to_update.collectors_item_type = collectors_item_data.get(
                "collectors_item_type"
            )
...
def delete_collectors_item_by_id(collectors_item_id):
    ...
    try:
        ...
        if not collectors_item_to_delete:
            warning_msg = "collectors_item with id {} not found.".format(
                collectors_item_id
            )
            logger.warning(warning_msg)
            response_object = {"status": "success", "message": warning_msg}
        ...
        response_object = {
            "status": "success",
            "message": "collectors_item with id {} successfully deleted".format(
                collectors_item_id
            ),
        }
        ...
        response_object = {
            "status": "failed",
            "message": "failed to delete collectors_item with id {}".format(
                collectors_item_id
            ),
        }
        return response_object, 500